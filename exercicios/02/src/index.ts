...
function savePlanet(name: string, coordinates: planetCoordinates, situation: string) {
    const planet = {
        name,
        coordinates,
        situation,
...
function updateSituation(newSituation: string, planet: { situation: string }) {
    planet.situation = newSituation;
    alert(`Situação alterada com sucesso`);
}

function addSatellites(newSatellite: string, planet: { satellites: string[] }) {
    planet.satellites.push(newSatellite);

    alert(`Satélite adicionado com sucesso`);
}

function remSatellites(satellite: string, planet: { satellites: string[] }) {
